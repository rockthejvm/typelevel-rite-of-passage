  ...
  override def all(filter: JobFilter, pagination: Pagination): F[List[Job]] = {
    ...
    val whereFragment: Fragment = Fragments.whereAndOpt(
      ...
      filter.tags.toNel.map(tags =>
        // any of the column tags
        // list of fragments which I need to expand into varargs: composite OR filter for tags
        // intersection between filter.tags and row's tags
